...
다중 프로세스: 빌드 후 `scripts/NetworkSmoke.ps1`. 호스트+클라이언트 3개를 숨긴 창으로 실행한다. 자동 입력은 실제 입력 메시지 경로로 이동/대피/진화를 요청한다. 약 110초 후 공유 경험치 210, 해금 2, 성장기 복귀, 4명 실제 접속, 구조 승리, 악당 보고 격리, 액터 HP/단계/에너지 일치를 검사한다. 결과 `.verification/network/`. `-Player <빌드 실행 파일>`, `-Port <빈 UDP 포트>`로 변경 가능.
