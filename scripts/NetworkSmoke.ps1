...
$output = Join-Path $repo '.verification/network'
New-Item -ItemType Directory -Force -Path $output | Out-Null
$hostArgs = @('-batchmode', '-nographics', '-digiHost', '-digiAuto', '-digiPort', $Port, '-digiQuit', '110', '-digiTrace', ('"' + (Join-Path $output 'host.json') + '"'), '-logFile', ('"' + (Join-Path $output 'host.log') + '"'))
$hostProcess = Start-Process -FilePath $Player -ArgumentList $hostArgs -WindowStyle Hidden -PassThru
Start-Sleep -Seconds 3
$clients = @()
for ($i = 1; $i -le 3; $i++) {
    $arguments = @('-batchmode', '-nographics', '-digiClient', '127.0.0.1', '-digiAuto', '-digiPort', $Port, '-digiQuit', '100', '-digiTrace', ('"' + (Join-Path $output "client$i.json") + '"'), '-logFile', ('"' + (Join-Path $output "client$i.log") + '"'))
